...
int file_uploaded(char* recfile_path, char *main_jconf, char *gmm_jconf, char *wav_config) {
	...
	char _argv0[] = "julius";
	char _argv1[] = "-C";
	char _argv2[] = "main.jconf";
	char _argv3[] = "-C";
	char _argv4[] = "am-gmm.jconf";
//	char _argv5[] = "-nolog";
	_argv[0] = _argv0;
	_argv[1] = _argv1;
	_argv[2] = _argv2;
...
//	_argv[5] = _argv5;
	...
	ffmpeg_convert_voice_file2wav(recfile_path);
	...
	fp = fopen("main.jconf", "w");
	fputs(main_jconf, fp);
	fflush(fp);
	fclose(fp);

	fp = fopen("am-gmm.jconf", "w");
	fputs(gmm_jconf, fp);
	fflush(fp);
	fclose(fp);
